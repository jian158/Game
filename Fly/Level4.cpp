...
void Level4::Updata()
{
	...
	if (autoShoot&&aShoot.getLength()>300)
	{
		aShoot.Reset();
	...
	if (timer_ecreate.getLength()>5000)
	{
		timer_ecreate.Reset();
		CreateEneMy(Num_Enemy, 1);
		CreateEneMy(Num_Enemy, -1);
...
void initSpShip()
{
	...
	spShip->dirCurr = PI / 2;
	spShip->live = 100;
	spShipBullet = --spShipBullet<1 ? 1 : spShipBullet;
}
